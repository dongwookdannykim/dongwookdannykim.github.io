---
layout: about
title: about
permalink: /
subtitle: Ph.D. Candidate, Department of Political Science, <a href="https://cla.umn.edu/polisci">University of Minnesota</a>

profile:
  align: right
  image: headshot.PNG
  image_circular: false # crops the image to make it circular
  more_info: 

selected_papers: false # includes a list of papers marked as "selected={true}"
social: false # includes social icons at the bottom of the page

announcements:
  enabled: false
  scrollable: true
  limit: 5
  
latest_posts:
  enabled: false
  scrollable: true
  limit: 3
---

I am a Ph.D. candidate in Political Science at the University of Minnesota, specializing in international relations and political methodology. I study how states build authority in spaces that do not belong to any state, such as the high seas, international airspace, outer space, and other parts of the global commons.

I came to these questions through a broader interest in the places where international order is made but not easily named. Some forms of authority are clearly territorial. Others are delegated to international institutions. But many important arrangements sit somewhere in between. They are partial, contested, and often ambiguous, yet they can still become durable parts of world politics. My dissertation, *Authority without Territory*, explores how that happens through cases that run from the Exclusive Economic Zone to Air Defense Identification Zones to a failed claim on the geostationary orbit whose project outlived it.

What interests me most is not simply whether a state is powerful enough to control a space, but how control becomes intelligible as a right. I ask how states justify new claims, how others learn to live with or resist them, and how the form a claim takes makes some arrangements easier to sustain than others. And because arrangements like these tend to outlast the claims that made them, I also ask who acquires the authority to correct and how.

Before coming to Minnesota, I received an M.A. in Security Studies from Georgetown University and B.A. degrees in Political Science and International Relations and in Sociology from Seoul National University, where I also completed a minor in Economics. I previously worked as a translator, an experience that continues to inform how I think about concepts as they move across languages, disciplines, and the worlds of scholarship, policy, and law. A full CV is available [here](/cv/). 

Outside research, I keep wandering into maps, book arts, coffee and tea, music, theater, birdwatching, and tabletop role-playing games — small worlds that reward slow attention.
